...
st.caption("AI-Powered Talent Acquisition Engine with Ethical Blind Anonymization & Explainable AI (XAI)")
...
st.sidebar.subheader("Engine AI & Model")
...
if provider_choice == "Google Gemini":
    ...
    selected_model = st.sidebar.selectbox(
        ...
        help="gemini-1.5-flash direkomendasikan untuk kecepatan ekstraksi dan efisiensi kuota."
    )
    ...
    api_key_input = st.sidebar.text_input(
        ...
        type="password",
        help="Dapatkan API key di Google AI Studio (aistudio.google.com)."
    )
    ...
    selected_model = st.sidebar.selectbox(
        ...
        model_options,
        index=0,
        help="gpt-4o-mini direkomendasikan untuk kecepatan dan efisiensi biaya."
    )
    ...
    api_key_input = st.sidebar.text_input(
        ...
        type="password",
        help="Dapatkan API key di OpenAI Platform (platform.openai.com)."
    )
...
st.sidebar.markdown("---")
st.sidebar.subheader("Mitigasi Bias (Blind-CV)")

enable_blind_cv = st.sidebar.toggle(
    "Aktifkan Blind-CV",
    value=True,
    help="Menyamarkan data pribadi kandidat sebelum evaluasi untuk mencegah bias kognitif."
)

if enable_blind_cv:
    with st.sidebar.expander("Parameter yang Dianonimkan", expanded=False):
        st.checkbox("Nama Lengkap", value=True, key="chk_name")
        st.checkbox("Alamat Email", value=True, key="chk_email")
        st.checkbox("Gender / Jenis Kelamin", value=True, key="chk_gender")
        st.checkbox("Usia / Umur", value=True, key="chk_age")
        st.checkbox("Alamat Domisili", value=True, key="chk_address")
        st.checkbox("Foto Profil", value=True, key="chk_photo")
        st.checkbox("Nama Universitas / Almamater", value=True, key="chk_univ")

st.sidebar.markdown("---")
st.sidebar.subheader("Kriteria Penilaian")

min_score = st.sidebar.slider(
    "Ambang Batas Kelulusan (Pass Threshold %)",
    min_value=0,
    max_value=100,
    value=60,
    step=5,
    help="Kandidat dengan skor di atas ambang batas ini akan dikategorikan sebagai 'Pass'."
)
...
# ==========================================
st.header("1. Pengaturan Posisi & Kriteria Lowongan (Job Description)")

jd_input_mode = st.radio(
    "Pilih Metode Input Job Description:",
    ["Upload Dokumen PDF", "Ketik / Tempel Teks Langsung"],
    horizontal=True
)
...
if jd_input_mode == "Upload Dokumen PDF":
    ...
    if jd_raw_text and len(jd_raw_text.strip()) >= 20:
        ...
        st.warning("Teks terlalu pendek. Masukkan informasi posisi dan kualifikasi lowongan secara lebih lengkap.")
    else:
        st.info("Silakan ketik atau tempel teks rincian lowongan pekerjaan pada kotak di atas.")
...
# ==========================================
st.header("2. Pengumpulan & Upload CV Kandidat")
...
with col_clear_btn:
    if st.button("Hapus Semua CV", help="Klik untuk menghapus/mereset seluruh berkas CV yang telah diunggah."):
    ...
    with st.spinner(f"AI ({provider_choice}) sedang memvalidasi dan mengekstrak seluruh PDF CV..."):
        for cv_file in uploaded_cv_files:
            try:
                ...
                st.warning(f"File Dilewati [{cv_file.name}]: Berkas kosong atau scan gambar tanpa teks digital.")
                invalid_cv_count += 1
            except InvalidDocumentError as e:
                st.warning(f"File Dilewati [{cv_file.name}]: {str(e)}")
                invalid_cv_count += 1
            except Exception as e:
                st.warning(f"Gagal Memproses [{cv_file.name}]: {str(e)}")
                invalid_cv_count += 1
...
if candidates_to_process and active_job:
    ...
    tab1, tab2, tab3 = st.tabs(["Leaderboard & Hasil Seleksi", "Blind-CV Anonymization", "Distribusi Skor"])
    ...
    with tab1:
        ...
        with m2:
            with st.container(border=True):
                st.metric("Kandidat Lolos (Pass)", len(filtered_list))
        with m3:
            with st.container(border=True):
        ...
        for rank, item in enumerate(evaluated_results, start=1):
            ...
            with st.container(border=True):
                ...
                with st.expander("Lihat Analisis Transparan AI (Pros & Cons)"):
                    ...
                    st.markdown("**Catatan / Potensi Gap (Cons):**")
                    for con in item["justification"]["cons"]:
                        st.markdown(f"- {con}")
                st.markdown("---")

    with tab2:
        st.subheader("Blind-CV Anonymization (Konfigurasi & Audit Bias)")
        st.info("Fitur ini menyamarkan informasi pribadi (PII) sebelum data dikirim ke sistem penilaian AI, memastikan evaluasi 100% berbasis kompetensi & rekam jejak.")
        
        with st.container(border=True):
            st.markdown("#### Parameter Masking Aktif:")
            ...
            st.caption("Ubah checklist melalui menu sidebar 'Parameter yang Dianonimkan' untuk menyesuaikan aturan secara real-time.")
        ...
        with col_left:
            st.error("Data Mentah CV Asli (Rentan Bias)")
            st.json(target_audit["raw_cv"].get("personal_info", {}))
        with col_right:
            st.success("Data Blind-CV yang Diterima AI (Sesuai Masking Aktif)")
    ...
    st.info("Silakan unggah dokumen PDF Job Description pada bagian (1) dan PDF CV kandidat pada bagian (2) di atas.")
elif not active_job:
    st.warning("Dokumen Job Description yang valid belum diunggah. Silakan unggah berkas PDF lowongan pada bagian (1).")
...
st.caption("Autonomous Candidate Screening Platform v1.7.0 | AI Specialist Technical Assessment")
